...
module.exports = function(Serviceport) {
  Serviceport.deleteMultiple = function(multiple,callback) {
    ...
    var sql ='DELETE FROM `ServicePort` where id in (' + multiple.join(',') +')';
    conn.executeSQL(sql, [], {}, function(err, back) {
         callback(err, back);
      });
    // }
  };
  Serviceport.remoteMethod('deleteMultiple', {
    accepts: [
      {
          arg: 'multiple',
          type: '[number]'
...
console.log(one);
  ...
  var pageCount=0;//总页数
  Serviceport.count( {}, function (err, result) {
    //console.log(result);
  ...
  //console.log(startpage);
    Serviceport.find({limit:DisplayEveryPage, skip:startpage},
      function (err, result) {
        if (!err) {
