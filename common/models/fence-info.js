...
module.exports = function(Fenceinfo) {
  Fenceinfo.deleteMultiple = function(multiple,callback) {
    ...
    var sql ='DELETE FROM `Fenceinfo` where id in (' + multiple.join(',') +')';
    conn.executeSQL(sql, [], {}, function(err, back) {
         callback(err, back);
      });
    // }
  };
  Fenceinfo.remoteMethod('deleteMultiple', {
    accepts: [
      {
          arg: 'multiple',
          type: '[number]'
  ...
  // 模糊查询  rodename    精确查询  classification
    Fenceinfo.FuzzyPrecision = function(Name,AdministrativeDepartment,callback) {
          Fenceinfo.find({where: {or:[{Name:{like: '%'+Name+'%'}},{AdministrativeDepartment:{like: '%'+AdministrativeDepartment+'%'}}]}},
                  function (err, result) {
                    if (!err) {
      ...
      console.log(one);
        ...
        var pageCount=0;//总页数
        Fenceinfo.count( {}, function (err, result) {
          //console.log(result);
        ...
        //console.log(startpage);
          Fenceinfo.find({limit:DisplayEveryPage, skip:startpage},
            function (err, result) {
              if (!err) {
