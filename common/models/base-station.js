...
module.exports = function(Basestation) {
  Basestation.deleteMultiple = function(multiple,callback) {
    ...
    var sql ='DELETE FROM `BaseStation` where id in (' + multiple.join(',') +')';
    conn.executeSQL(sql, [], {}, function(err, back) {
         callback(err, back);
      });
    // }
  };
  Basestation.remoteMethod('deleteMultiple', {
    accepts: [
      {
          arg: 'multiple',
          type: '[number]'
...
console.log(one);
  ...
  var pageCount=0;//总页数
  Basestation.count( {}, function (err, result) {
    //console.log(result);
  ...
  //console.log(startpage);
    Basestation.find({limit:DisplayEveryPage, skip:startpage},
      function (err, result) {
        if (!err) {
