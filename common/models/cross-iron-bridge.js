...
module.exports = function(Crossironbridge) {
  Crossironbridge.deleteMultiple = function(multiple,callback) {
    ...
    var sql ='DELETE FROM `CrossIronBridge` where id in (' + multiple.join(',') +')';
    conn.executeSQL(sql, [], {}, function(err, back) {
         callback(err, back);
      });
    // }
  };
  Crossironbridge.remoteMethod('deleteMultiple', {
    accepts: [
      {
          arg: 'multiple',
          type: '[number]'
